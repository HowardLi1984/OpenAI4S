...
- **`openai4s/sdk/host.py`** — the `host` facade injected into the python kernel. Methods route through `host_call(method, args)` — EXCEPT `host.bash`, which is **kernel-local**: it runs `subprocess` inside the worker process (cwd = session workspace, PATH already carries the active env's bin) with the static shell precheck + egress scan, because the host process executes only python/R cells, never shell.
